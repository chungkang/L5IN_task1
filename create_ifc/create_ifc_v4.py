...
Original file is located at
    https://colab.research.google.com/drive/10QURT3GeLTyO2l56kZP7DUY-_Rb_0I8N
"""
...
def get_angle(p0_x, p0_y, p1_x, p1_y):
...
filename = "floorplan.ifc"
timestamp = time.time()
timestring = time.strftime("%Y-%m-%dT%H:%M:%S", time.gmtime(timestamp))
creator = "VS"
organization = "HCU"
application, application_version = "IfcOpenShell", "0.6"
project_globalid, project_name = create_guid(), "4th Floor HCU Test"
...
with open("walls/walls_v3.txt") as f:
    for co, line in enumerate(f):
        line = line.split()

        #l = float(line[4]) + t #wall lenght(M)
        ...
        x1 = float(line[2])
        y1 = float(line[3])
        print ('Line from ( {:.2f}| {:.2f} ) to ( {:.2f} | {:.2f} )'.format(x0, y0, x1, y1))
        line_angle = get_angle(x0, y0, x1, y1)

        #Vlad edit: This allows the drawing of lines

        #Ref: https://stackoverflow.com/questions/7586063/how-to-calculate-the-angle-between-a-line-and-the-horizontal-axis

        #get the direction vector
        dir_vec = (x1 - x0, y1 - y0, 0.0)
        
        #calculate length of the line, z can be ignored here
        l = math.sqrt(dir_vec[0]*dir_vec[0]+dir_vec[1]*dir_vec[1])

        s0 = (x0, y0, 0.0) #start position(x,y,z)
        s1 = (x1, y1, 0.0) #end position(x,y,z)
        
        wall_placement = create_ifclocalplacement(ifcfile, relative_to=storey_placement)
        polyline = create_ifcpolyline(ifcfile, [s0, s1])
        axis_representation = ifcfile.createIfcShapeRepresentation(context, "Axis", "Curve2D", [polyline])

        extrusion_placement = create_ifcaxis2placement(ifcfile, s0, Z, dir_vec)

        point_list_extrusion_area = [(0.0, -t/2, 0.0), (l, -t/2, 0.0), (l, t/2, 0.0), (0.0, t/2, 0.0), (0.0, -t/2, 0.0)]
        solid = create_ifcextrudedareasolid(ifcfile, point_list_extrusion_area, extrusion_placement, Z, h)
...
print("Writing floorplan to IFC file..")
